...
(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find action '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,action,exports){
"use strict";

/* start of gexrender script */
...
gexrender.typesMatch = function (types, layer) {
    return gexrender.types.filter(function (t) {
        return layer instanceof t;
    }).length > 0;
};

gexrender.replaceFootage = function (layer, filepath) {
    if (!layer) { return false; }

    var file = new File(filepath);

    if (!file.exists) {
        throw new Error("gexrender: Trying to create a file replacement for an unknown file: " + filepath);
    }
...
gexrender.selectCompositionsByName = function(name, callback) {
    var items = [];
    ...
    if (len == 0) {
        throw new Error("gexrender: Cound't find any compositions by provided name (" + name + ")");
    }
};

/* call callback for an every layer matching specific name and composition */
gexrender.selectLayersByName = function(compositionName, name, callback, types) {
    var foundOnce = false;

    if (!compositionName) compositionName = gexrender.defaultCompositionName;
    if (!types) types = gexrender.types;

    gexrender.selectCompositionsByName(compositionName, function(comp) {
        var items = [];
        ...
        for (var j = 1; j <= comp.numLayers; j++) {
            ...
            if (gexrender.typesMatch(types, layer)) {
                foundOnce = true;
                items.push(layer);
            }
        }
    ...
    if (!foundOnce) {
        throw new Error("gexrender: Cound't find any layers by provided name (" + name + ") inside a composition: " + compositionName);
    }
};

/* call callback for an every layer matching specific index and composition */
gexrender.selectLayersByIndex = function(compositionName, index, callback, types) {
    var foundOnce = false;
    ...
    if (!foundOnce) {
        throw new Error("gexrender: Cound't find any layers by provided index (" + index + ") inside a composition: " + compositionName);
    }
};

/* ensure that eval only gets a minimal variable scope */
gexrender.evaluate = function (layer, expression) {
    return eval(expression);
}

gexrender.changeValueForKeypath = function (layer, keys, val) {
            ...
            if ("property" in o && o.property(key)) {
                ...
                if (!v.changed) {
                    if ("value" in v) {
                        ...
                        o[key] = gexrender.evaluate(layer, v.expression)
                    }
                }
                return { "value": o, "changed": true };
            } else {
                throw new Error("gexrender: Can't find a property sequence " + keys.join('.') + " for key: " + key);
            }
        }
    }
